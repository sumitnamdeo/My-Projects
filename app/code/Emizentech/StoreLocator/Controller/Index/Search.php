...
// 		}
		...
		$query = sprintf("SELECT address, title , latitude , longitude , ( 3959 * acos( cos( radians('%s') ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians('%s') ) + sin( radians('%s') ) * sin( radians( latitude ) ) ) ) AS distance FROM ".$storeTable." where is_active = 1  HAVING distance < '%s' ORDER BY distance LIMIT 0 , 25 ",
		  ($center_lat),
		  ($center_lng),
		  ($center_lat),
		  ($radius));
